...
class AuthUtil {
  ...
  Future<User> signInWithUserNameAndPassword(
    ...
    Map<String, String> headers = {
      'Authorization':
          'DirectLogin username="$userName",password="$password",consumer_key="${constants.consumerKey}"',
      'Content-Type': 'application/json'
    };
    ObpResponse response;
    try {
      response =
          await httpRequest.post(constants.directLoginUrl, headers: headers);
  ...
  Future<List<Entitlement>> getEntitlements() async {
    ObpResponse response =
        await httpRequest.get(constants.currentUserUrl, headers: authHeaders);
