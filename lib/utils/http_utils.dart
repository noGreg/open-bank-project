...
class HttpRequest {
  ...
  ObpResponse _extractResponse(Response response) {
    if (200 <= response.statusCode && response.statusCode <= 299) {
      ...
      try {
        ...
        return ObpResponse(
            code: errorMessage.code, message: errorMessage.message);
      } catch (e) {
        return ObpResponse(
            code: response.statusCode, message: 'Unknown Server Error.');
      }
    }
  }
  ...
  Future<ObpResponse> get(String url,
    ...
    Response response =
        await http.get(_buildUrl(url, params), headers: headers);
  ...
  Future<ObpResponse> delete(String url,
      {Map<String, String> headers, Map<String, dynamic> params}) async {
    Response response =
        await http.delete(_buildUrl(url, params), headers: headers);
    return _extractResponse(response);
  }
  ...
  Future<ObpResponse> post(String url,
    ...
    try {
      response =
          await http.post(_buildUrl(url, params), headers: headers, body: data);
    } catch (e) {
      print(e);
    }
    return _extractResponse(response);
  ...
  Future<ObpResponse> put(String url,
      ...
      Map<String, dynamic> params}) async {
    Response response =
        await http.put(_buildUrl(url, params), headers: headers, body: data);
    return _extractResponse(response);
  }
}
